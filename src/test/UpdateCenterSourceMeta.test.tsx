...
describe("Update Center source metadata", () => {
  it("shows repository, path, and URL for updatable skills", () => {
    ...
    expect(
      within(row).getByText("skills/planning-with-files-zh"),
    ).toBeInTheDocument();
    expect(
      within(row).getByText("https://github.com/openai/skills"),
    ).toBeInTheDocument();
  ...
  it("uses a uniform neutral treatment for all source metadata kinds", () => {
    render(
      <UpdatableTabPanel
        items={[updatable]}
        state={{ "planning-with-files-zh": { selected: true } }}
    ...
    expect(metaChip("repository")).toHaveClass("bg-muted/35", "ring-border/60");
    expect(metaChip("path")).toHaveClass("bg-muted/35", "ring-border/60");
    expect(metaChip("url")).toHaveClass("bg-muted/35", "ring-border/60");
    expect(metaChip("cache")).toHaveClass("bg-muted/35", "ring-border/60");
    expect(metaChip("hash")).toHaveClass("bg-muted/35", "ring-border/60");
  });
  ...
  it("falls back to the repository id when repository metadata is missing", () => {
    ...
    const row = screen.getByText("planning-with-files-zh").closest("label")!;
    expect(
      within(row).getByText("github-openai-skills-main"),
    ).toBeInTheDocument();
  });
});
