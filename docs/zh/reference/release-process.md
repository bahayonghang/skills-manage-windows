...
- 触发方式：推送已存在的 `v<semver>` tag，或手动选择一个已存在 tag。
...
6. Windows release-only config 和 `release-assets/` 准备好后，运行：
   - `pnpm release:preflight -- --version <version> --tag v<version> --config release-updater-config.json --asset-dir release-assets`
   - 该命令会检查 updater 配置和 `latest.json`，并使用 updater 公钥对
     NSIS 实际字节做密码学验签。
7. 将发布提交合入 `main`。
8. 在该 `main` commit 创建并推送 `v<version>`。重试时可手动触发
   `Release Desktop` 并填写同一个已存在 tag。
9. 等待 frozen context、可复用 CI 和所有必需平台构建完成。workflow 在创建
   或复用 draft 前验证完整产物清单、updater 签名、metadata 和
   `SHA256SUMS`。
10. 确认原子公开后的 GitHub Release 包含：
   - `latest.json`
...
- 正式 release workflow 必须注入真实 updater public key，并为 Windows 构建
  开启 updater artifacts，然后通过密码学 preflight。
