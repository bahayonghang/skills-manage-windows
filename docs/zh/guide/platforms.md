...
平台是指任何会读取 `skills/` 目录的 AI 编码 agent 或运行时。SkillPort 预置 27 个平台定义，并允许在设置中添加自定义平台。
...
Codex、Cursor、Gemini CLI、OpenCode、Amp、Copilot 都读取共享的 Universal Agents 路径。安装一次即对它们全部生效。
