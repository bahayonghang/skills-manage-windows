...
pub(crate) fn project_global_inventory(
    ...
    for (name, entry) in ownership.iter() {
        ...
        skills.push(skill_from_lock_entry(
            name,
            entry,
            &canonical.to_string_lossy(),
            is_owned_directory(&canonical),
            placements,
        ));
    }
    skills
...
pub(crate) fn skill_from_lock_entry(
    name: &str,
    entry: &CliLockEntry,
    canonical: &str,
    canonical_owned: bool,
    placements: Vec<SkillsCliPlacement>,
    ...
    let (path, install_kind) = if canonical_owned {
        (Some(canonical.to_string()), SkillsCliInstallKind::Canonical)
