...
pub(super) const REMOVE_LOCK_OPERATION: &str = "Skills CLI global remove";
pub(super) const MANIFEST_VERSION: u32 = 1;
pub(super) const PHASE_PREPARED: &str = "prepared";
pub(super) const PHASE_STAGED: &str = "staged";
pub(super) const PHASE_METADATA_COMMITTED: &str = "metadata_committed";
const TEMP_PREFIX: &str = ".skillport-skills-cli-lock-";
pub(super) const LOCK_READ_LIMIT: u64 = 1_048_576;
...
pub(super) fn injected_fault() -> Option<RemoveFault> {
    #[cfg(test)]
    {
...
pub(super) struct ManagedLinkRecord {
    agent_id: String,
    path: String,
    kind: String,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub(super) struct RemoveManifestV1 {
    version: u32,
    operation_id: String,
    skill_name: String,
    phase: String,
...
pub(crate) async fn preview_remove_global(
    ...
    if tx.is_remote() {
        return remote::preview_remove_global_remote(tx, pool, skill_name).await;
    }
    let paths = tx.paths();
...
pub(crate) async fn recover_pending_via_transport(
    ...
    remote::recover_manifest_via_transport(tx, &path).await
}
...
pub(super) fn plan_from_classified(
    skill_name: &str,
