...
pub(crate) async fn resolve_repo_source(
    ...
    resolve_repo_source_with_branch(repo_url, None, auth_token).await
}

pub(crate) async fn resolve_repo_source_with_branch(
    repo_url: &str,
    selected_branch: Option<&str>,
    auth_token: Option<&str>,
) -> Result<ResolvedGitHubRepoSource, GithubImportError> {
    let parsed = parse_github_source(repo_url)?;
    let selected_branch = reconcile_selected_branch(parsed.branch.as_deref(), selected_branch)?;
    ...
    let branch = payload
        ...
        .unwrap_or("main")
        .to_string();
    let branch = selected_branch.unwrap_or(branch);
