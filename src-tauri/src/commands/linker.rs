...
use crate::targets::{
    connect_ssh_target, remote_join, remote_symlink_allowed, ActiveTarget, ConnectedSshTarget,
    RemoteTargetConfig,
};
use crate::AppState;

fn batch_operation_status(success_count: usize, failure_count: usize) -> &'static str {
...
const REMOTE_CENTRAL_INSTALL_SCRIPT: &str = r#"
set -eu
...
async fn install_skill_to_agent_ssh_with_connection(
    pool: &DbPool,
    connection: &ConnectedSshTarget,
    target: &RemoteTargetConfig,
    skill_id: &str,
...
pub async fn batch_install_to_agents(
    ...
    let status = batch_operation_status(succeeded.len(), failed.len());
    record_operation_log_best_effort(
        &state.db,
        target_context,
        OperationLogEvent::new(
...
pub async fn batch_install_central_skills(
    ...
    if let ActiveTarget::Ssh(target) = active_target {
        ...
        for skill_id in skill_ids {
            for agent_id in &agent_ids {
                match install_skill_to_agent_ssh_with_connection(
                    &pool,
                    &connection,
                    &target,
                    &skill_id,
    ...
    let batch_result = batch_install_central_skills_impl(
        &pool,
        skill_ids,
        agent_ids,
        method,
    ...
    .await;
    let status = batch_operation_status(batch_result.succeeded.len(), batch_result.failed.len());
    record_operation_log_best_effort(
        &state.db,
        target_context,
        OperationLogEvent::new(
