...
pub async fn test_ssh_target(
    ...
    test_ssh_target_impl(&state.db, request).await
}
...
pub async fn update_ssh_target_password(
    ...
    update_ssh_target_password_impl(&state.db, &target_id, &password).await
}
...
pub async fn set_active_target(
    ...
    set_active_target_impl(&state.db, &target_id).await
}

#[tauri::command]
pub async fn get_active_target(state: State<'_, AppState>) -> Result<TargetSummary, String> {
    get_active_target_impl(&state.db).await
}
