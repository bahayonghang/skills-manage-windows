...
```text
web: typecheck -> lint -> capabilitycheck -> sizecheck -> test -> build -> docs:build
...
- `pnpm docs:build` runs the read-only check before VitePress. `scripts/run-ci.mjs` includes that build in the Web chain, so routine pull requests validate both drift and site bundling.
