...
Runtime read DTOs expose optional `operationId` and `eventSource` (`backend | frontend`). Frontend recorder payloads
accept an optional UUID `operationId`; Rust independently validates every renderer-originated field before tracing.
...
- `record_runtime_failure` reuses an operation ID for Operation commands and creates a same-format ephemeral ID for
  ...
  silent. Backend events include only reviewed command/category/phase/code/retryable/target kind, duration, correlation,
  and `event_source=backend`.
- The frontend recorder creates a UUID when a legacy rejection lacks backend correlation and records
  `correlationOrigin=frontend`; otherwise it preserves the backend ID and origin. Concurrent or indefinitely pending
  recorder writes must not suppress later evidence, and recorder failure never changes the user operation result.
- Renderer-originated events use fixed source-specific messages. IPC details retain only a registered command, ordinalized
  argument shape/non-string scalars, a reviewed stable code/retryable flag and correlation origin. Global error events
  retain only an allowlisted error name and nonnegative line/column; raw message, filename, stack, reason and dynamic code
  are discarded. Rust repeats this fail-closed normalization before the event reaches the tracing subscriber.
- Runtime parsing preserves historical lines, extracts valid snake/camel case operation/event-source fields, and applies
  operation ID and event-source filters by exact match. A nonempty invalid UUID filter matches zero rows.
- Backend and frontend Runtime events are distinct sources sharing the ID, not storage duplicates to delete.
- Reviewed IPC error codes are Rust-owned and code-generated to the renderer. Both sides reject syntax-valid codes outside
...
- Good: a target-aware failure uses
  `ipc_boundary!("command", target_kind = OperationTargetKind::Ssh, expression)` and logs no target address.
...
- Runtime boundary: sync/async named forms, exactly-once failure evidence, target-kind local/SSH/WSL propagation,
  Operation ID reuse, Runtime-only UUID generation and Excluded silence.
- Renderer ingress: concurrent/pending/failing writes, legacy/unknown correlation fallback, source/message/code allowlists,
  object-key adversarial seeds, exact correlation/event-source filters and invalid-filter zero-match behavior.
...
// Wrong: the compatibility boundary cannot resolve policy or target kind.
ipc_boundary!(service_call())
...
// Correct: a fallible target-aware IPC rejection has one typed Runtime boundary.
ipc_boundary!(
    "get_central_skills",
