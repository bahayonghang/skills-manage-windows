...
Shared Zustand state is `githubWizardOpen`, `githubSource`, `githubBranch`, and
`pendingSources` (FIFO, maximum 8). `githubBranch` is presentation state only;
the backend remains the branch parser and validator.
...
- Dirty means non-empty source, non-empty manual branch, or any preview/loading/error/import/result state. A new source never overwrites a dirty session; it enters a deduplicated FIFO of at most 8.
- Pending UI shows count. The user closes the current wizard before consuming the FIFO head, or discards an item. Consume clears the old GitHub session, including `githubBranch`, but does not preview.
- Directly opening or consuming a deep-link source clears `githubBranch`; a `/tree/<branch>` URL must not inherit the previous session's manual branch. Wizard reset/close, start-another-import, test reset, and target change clear the manual branch at the same lifecycle boundary as GitHub import state.
- Native intent payload remains exactly `{ source }`. Do not add a second branch field to the event until native senders, validation, queue identity, and conflict behavior have a separate reviewed contract.
...
| branch is non-empty while source/preview are empty | treat session as dirty; queue a new deep link |
| consume or directly open a new deep-link source | clear manual branch before opening; URL carries any branch intent |
...
- Store/UI: source-only and branch-only dirty session preservation, duplicate,
  FIFO overflow, pending count, close/consume/discard, and branch clearing on
  deep-link consumption/reset.
