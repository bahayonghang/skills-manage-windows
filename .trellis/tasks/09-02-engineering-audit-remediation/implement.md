...
7. [x] （R1）ledger **29** 个 ID：27 `fixed`（含 QUAL-002 contract/fixture、QUAL-SIZE-001、ARCH-003/004 渐进），2 `open (fail-closed)`（REL-001/REL-002）。未标 `wontfix`。
8. [x] （R4）父任务 planning artifacts 入库；**不**归档父任务、**不** push。
9. [x] 原始扫描信封复跑：`research/scan-rerun-2026-09-03.md`。独立 trellis-check：**FINDINGS**（AC2 仍 blocked）。
