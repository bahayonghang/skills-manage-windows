...
- 2026-08-01 本机热缓存 `just ci` 通过，总耗时 130.3 秒；前端 1565 通过、1 跳过，Rust 主库 1031 通过、6 忽略，其他 CLI/E2E 路径通过。
- 最近三个成功 PR CI 用时 16.5、22.4、41.2 分钟。PR #26 在 runner 开始工作后仍按 Unix 前置约 11 分钟、Windows `just-ci` 约 15 分钟串行执行。
- 最近 15 个合并 PR 的中位数为 137 个文件、5716 行新增；9 个超过 100 个文件，4 个超过 400 个文件。
- `main` 受保护，稳定 required context 是 `just-ci`；`dev` 未保护，当前 CI 不在 `main` / `dev` push 上触发。
- `pnpm docs:build` 成功但会改写两份已跟踪生成文档；线上项目 Pages URL 返回 404，尽管 `gh-pages` 分支和历史成功 workflow 都存在。
...
8. GitHub Pages 设置、分支保护/合并策略、release environment、签名服务和任何公开发布都属于外部变更，执行前单独展示并确认精确副作用。
...
11. 文档部署 workflow 合并后，将仓库 Pages source 切换为 GitHub Actions，执行线上 HTTP 200 与 SkillPort 页面身份 smoke，并回读 Pages 设置；保留现有 `gh-pages` 分支，删除另行审批。
...
| `08-01-developer-pr-experience` | 工具链固定、分层本地命令、PR 模板、短分支直入 `main` 的分支/合并策略 | 第二批 |
...
- [ ] 按已授权方案将 Pages source 切换为 GitHub Actions 并回读验证；项目 Pages URL 返回 HTTP 200 且页面身份正确，部署后 smoke 失败会使部署 workflow 失败。
