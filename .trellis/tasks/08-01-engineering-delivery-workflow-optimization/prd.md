...
- [ ] 获得外部设置授权后，仓库允许 squash merge 与 merge commit、关闭 rebase merge 并自动删除已合并任务分支；`dev-safety` 不可 bypass 地禁止 force/delete，`dev-flow` 对常规 task PR 强制 PR/`just-ci`/linear history 且仅为 bookkeeping/exact fast-forward 提供受控 bypass，实际设置回读与目标合同一致。
