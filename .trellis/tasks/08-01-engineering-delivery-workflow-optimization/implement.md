...
5. 工作提交通过 task PR squash 合入 `dev`；归档和 journal 只记录实际交付提交，再由 `dev -> main` promotion PR 以 merge commit 晋级。promotion 后刷新并验证 merge parents，通过受控 bypass 无 force fast-forward `dev` 到 `main` merge SHA，再写最终证据。
...
- 已上线的 Pages 设置保持不变，legacy `gh-pages` 保持不存在；后续 Pages 变更、merge settings、两个 `dev` ruleset/bypass、分支保护、release environment、Azure 配置和任何 release/tag 写入在执行前再次请求精确授权。
