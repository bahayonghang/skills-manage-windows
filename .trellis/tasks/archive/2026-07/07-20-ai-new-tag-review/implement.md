...
- [x] `db/schema/metadata.rs`：ensure_column 加 `proposed_name` /
      `proposed_description`。
- [x] `db/repos/tags_repo.rs`：`get_pending_ai_tag_reviews` 改 LEFT JOIN +
      COALESCE + `is_proposal`；`replace_pending_ai_tag_reviews` 对 proposal
      行放开存在性校验；`db/types.rs` `SkillAiTagReview` 加 `is_proposal`。
- [x] DB 单测：旧库升级加列；proposal 行写入/读取；孤儿行过滤。
- 验证：`cd src-tauri && cargo test db::`

### Step 2：create_skill_tag 原子化

- [x] `tags_repo.rs`：ON CONFLICT(name) DO NOTHING + 按 name 回读；撞 id
      回退 UUID。
- [x] 单测：并发/重复同名创建幂等；异名撞 id 回退。
- 验证：`cargo test db::`

### Step 3：接受/跳过语义

- [x] `accept_ai_tag_reviews`：proposal 行接受时先创建 tag（design §4），链接
      挂实际 id；`skip` 保持无残留。
- [x] 单测：接受创建 + 链接；同名重复接受复用；跳过零残留。
- 验证：`cargo test db::`

### Step 4：解析、归一化与流水线

- [x] `types.rs`：`RawAiNewTagSuggestion` + envelope `new_tag` 字段 +
      `SkillTagProposal`；`SkillTagSuggestionResult`/progress payload 加可选
      `proposals`。
- [x] `prompt.rs`：重写提示词（design §5）；`parse_ai_tag_suggestions` 返回
      envelope（兼容旧格式）；新增 `resolve_proposal` 纯函数（归一化、超长
      丢弃、撞名降级）。
- [x] `mod.rs`：proposal 只进 pending review，永不自动应用；计入
      `low_confidence_count`。
- [x] 单测：五种响应解析（复用/提议/混合/空/旧格式）；撞名降级；proposal
      不自动应用。
- 验证：`cargo test ai_tagging`

### Step 5：前端最小适配

- [x] TS 类型 `isProposal`；review 面板 proposal badge「AI 新建标签」（i18n
      中英）。
- [x] Vitest：proposal 展示与接受/跳过交互。
- 验证：`pnpm typecheck && pnpm test`

### Step 6：spec 同步（必做）

- [x] `.trellis/spec/backend/central-skill-tags.md`：新增 proposal/review 契约
