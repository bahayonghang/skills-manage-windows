...
- 四个子任务现已全部归档。2026-07-19 最终验收曾将 `07-18-unified-skill-import` 恢复为原执行范围，修复原子恢复、Operation Log、Zustand 状态所有权、前端/端到端测试和错误脱敏/i18n 缺口后重新归档；其他三个子任务始终保持归档未修改。
- 父任务验收结果、命令和真实 Tauri 证据见 `research/final-cross-child-acceptance.md` 与 `research/unified-import-revalidation.md`。父任务当前为 `planning [4/4 done]`，最终跨子任务验收已通过，可以归档，但本轮未获得父任务 archive 的明确指令，因此继续保持 planning。
...
- [x] 4 个子任务分别通过审批、实施、验证和归档；统一 ZIP 子任务在修复原始契约缺口后重新归档，其他三项保持原归档状态。
...
- [x] 每个子任务最终通过定向测试、`git diff --check` 和 `just ci`；涉及打包的深链任务额外通过 Windows `pnpm tauri build` 并验证安装产物。统一 ZIP 修复补齐 36 个 Rust tests、118 个 focused frontend tests 和全量 CI。
- [x] 父任务完成最终跨子任务回归，确认 Central、Marketplace、Usage、Operation Logs、远程目标和技能详情无语义或视觉回退；当前工作树 Tauri Operation Logs 与 Skill Detail 真实截图已保存。
